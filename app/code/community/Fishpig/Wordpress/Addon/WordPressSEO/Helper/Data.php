...
class Fishpig_Wordpress_Addon_WordPressSEO_Helper_Data extends Fishpig_Wordpress_Helper_Plugin_Seo_Abstract
{
	...
	{
		$metaValues = new Varien_Object(array(
		...
		if ($meta = Mage::helper('wordpress')->getWpOption('wpseo_taxonomy_meta')) {
			if ($meta = @unserialize($meta)) {
				if (isset($meta['category']) && isset($meta['category'][$category->getId()])) {
					...
					if ($meta->getWpseoTitle()) {
						$metaValues->setTitle($meta->getWpseoTitle());
					}
					
					if ($meta->getWpseoDesc()) {
						$metaValues->setDescription($meta->getWpseoDesc());
					}
					
					if ($meta->getWpseoMetakey()) {
						$metaValues->setKeywords($meta->getWpseoMetakey());
					}
		...
		$this->_applyMeta($metaValues->getData());
